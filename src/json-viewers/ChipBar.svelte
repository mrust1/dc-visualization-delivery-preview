<script>
  ...
  let pos = {x: 0, y:0};
  function getXY(e){
    let element = compareElement.contains(e.target) ? compareElement : baseElement;
...
<svelte:window on:click={()=>showList = false} on:blur={()=>showList = false}/>
<div class="control">
  <span bind:this={baseElement} on:click={(e)=>{option = base; showList = true; pos = getXY(e); e.stopPropagation()}}>
  ...
  <span bind:this={compareElement} on:click={(e)=>{option = compare; showList = true; pos = getXY(e); e.stopPropagation()}}>
    <Chip type="compare" title="{$compare}" />
  </span>
</div>
{#if showList}
  <TypeList {pos} on:close={()=>{showList = false}} on:choose={(e)=>{showList = false; option.set(e.detail)}}/>
{/if}
<style>
  .control {
